...
export default function Home() {
  // DÜZELTME 1: API URL ARTIK TEMİZ STRING
  const API_URL = "[https://rolescope-backend.onrender.com](https://rolescope-backend.onrender.com)";
  ...
  const questions = [
    ...
    { id: 9, text: "Yeni ve karmaşık bir teknolojiyi öğrenmen gerektiğinde stratejin nedir?", options: ["Dokümantasyonu baştan sona okur, teoriyi oturturum.", "Hemen boş bir proje açar, bozarak ve deneyerek öğrenirim.", "Bilen birinden bana anlatmasını isterim veya eğitim videosu izlerim.", "Sadece o an işime yarayan kısmını öğrenir geçerim."] },
    { id: 10, text: "Bir ekibe liderlik etmen istense, tarzın hangisine yakın olurdu?", options: ["Önden koşar, tempoyu belirler ve herkesi sürüklerim.", "Arkada durur, ekibin engellerini kaldırır ve desteklerim (Servant Leadership).", "Görevleri net şekilde dağıtır ve sonuçları sıkı takip ederim (Mikro Yönetim).", "Vizyonu çizerim, gerisine karışmam (Özgürlükçü)."] },
    { id: 11, text: "Yoğun bir günün ardından enerjini nasıl toplarsın?", options: ["Tek başıma kalarak, oyun oynayarak veya kitap okuyarak.", "Arkadaşlarımla dışarı çıkarak ve sosyalleşerek.", "Spor yaparak veya fiziksel aktiviteyle.", "Mesleğimle ilgili hobi projeleriyle uğraşarak."] },
    { id: 12, text: "Aylarca emek verdiğin proje iptal edildi ve çöpe gitti. Tepkin ne olur?", options: ["Büyük hayal kırıklığı yaşar, motivasyonumu zor toparlarım.", "Hemen 'Buradan ne öğrendik?' diye bakar, tecrübeyi cebime koyarım.", "Kararı verenleri sorgular ve itiraz ederim.", "Umursamam, maaşım yattığı sürece bir sonraki göreve bakarım."] }
  ];
  ...
  const handleAnalyze = async () => {
    // CV sekmesindeysek dosya zorunlu
    if (activeTab === "cv" && !file) {
        return alert("Lütfen devam etmeden önce CV dosyanızı yükleyin.");
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.detail || "Analiz sunucusu yanıt vermedi.");
      }
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans selection:bg-slate-200">
      ...
      <div className="bg-slate-900 text-white text-center py-3 px-4 text-xs font-medium tracking-wide relative z-[60] flex items-center justify-center gap-2">
        ...
        <span>🚀 RoleScope AI: <span className="text-emerald-400 font-bold">Premium Kariyer Analizi</span> sınırlı süre için ücretsiz.</span>
      </div>
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-4">
             {/* DÜZELTME 5: Butonlarda setFile(null) çalışıyor */}
             <button onClick={() => { setActiveTab("cv"); setResult(null); setFile(null); }} className={`text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === 'cv' ? 'text-slate-900 bg-slate-100 px-3 py-2 rounded-lg' : 'text-slate-400 hover:text-slate-600 px-3 py-2'}`}><FileText size={16} /> CV Analizi</button>
             <button onClick={() => { setActiveTab("dna"); setResult(null); setFile(null); }} className={`text-sm font-medium transition-colors flex items-center gap-2 ${activeTab === 'dna' ? 'text-slate-900 bg-slate-100 px-3 py-2 rounded-lg' : 'text-slate-400 hover:text-slate-600 px-3 py-2'}`}>
      ...
      <main className="container mx-auto px-6 py-12 max-w-6xl">
                ...
                <div className="lg:col-span-5 space-y-6">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm leading-relaxed">
                            {activeTab === 'cv' ? "Yapay zeka ile CV'nizi ATS standartlarına göre test edin." : "Kişilik özelliklerinizi analiz ederek en uygun kariyer yolunu keşfedin."}
                        </p>
                    </div>
                    ...
                    <div className="border-2 border-dashed border-slate-200 bg-slate-50/50 rounded-xl p-8 hover:border-slate-400 transition-all group cursor-pointer relative">
                        ...
                        <div className="flex flex-col items-center text-center">
                            ...
                            <p className="font-medium text-slate-900">
                                {activeTab === 'cv' ? "CV Dosyasını Yükle (Zorunlu)" : "CV Dosyasını Yükle (Opsiyonel)"}
                            </p>
                            <p className="text-xs text-slate-400 mt-1">
                                {activeTab === 'cv' ? "PDF veya DOCX" : "Daha net sonuç için CV önerilir"}
                            </p>
                            ...
                            {!testCompleted ? (
                                <div>
                                    <div className="flex justify-between items-center mb-8">
                                        <span className="text-xs font-bold tracking-widest text-amber-600 uppercase flex items-center gap-1"><Crown size={12} /> Premium Envanter (12 Soru)</span>
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-center space-y-6">
                                    ...
                                    <div><h3 className="text-xl font-bold text-slate-900">Profilin Hazır</h3><p className="text-slate-500 mt-2 text-sm">Cevapların kaydedildi. {file ? "CV ile sentezleniyor." : "Kişilik analizi oluşturuluyor."}</p></div>
        ...
        {/* SONUÇ EKRANLARI AYNI KALDI (CV ve DNA) */}
        {result && activeTab === 'cv' && (
            <div className="space-y-8 animate-fade-in">
                ...
                {/* ... (CV Grafik ve Sonuç Kodları Buraya Gelecek - Değişmedi) ... */}
                {/* Yer kaplamasın diye burayı kısalttım, önceki kodla birebir aynı */}
        ...
        {result && activeTab === 'dna' && (
            <div className="space-y-12 animate-fade-in relative">
                ...
                <div className="text-center max-w-3xl mx-auto">
                 ...
                 {/* ... Diğer DNA sonuç kartları (Önceki kodla aynı) ... */}
                    ...
                    <div className="bg-white border border-slate-200 rounded-xl p-8 shadow-sm">
                        ...
                        <div className="space-y-5">
                            {result.calisma_dinamigi && Object.entries(result.calisma_dinamigi).map(([key, value], index) => (
                                <div key={index}>
                                    <div className="flex justify-between text-[10px] text-slate-400 uppercase font-bold mb-2"><span>{key.replace('_', ' ')}</span></div>
